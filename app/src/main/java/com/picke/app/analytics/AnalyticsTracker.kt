package com.picke.app.analytics
...
class AnalyticsTracker @Inject constructor(
    ...
    private fun registerBaseSuperProperties() {
        safely("registerBaseSuperProperties") {
            mixpanel.registerSuperProperties(JSONObject().apply {
                ...
                put(AnalyticsProp.APP_VERSION, BuildConfig.VERSION_NAME)
                put(AnalyticsProp.BUILD, BuildConfig.VERSION_CODE.toString())
            })
        ...
        safely("onLogin") {
            ...
            if (isNewUser) {
                mixpanel.people.setOnce(JSONObject().apply {
                    put(AnalyticsProp.SIGNUP_DATE, DateTimeFormatter.ISO_INSTANT.format(Instant.now()))
                })
    ...
    fun trackBattleStep(stepName: String, contentId: String, choice: String? = null, isChanged: Boolean? = null) {
